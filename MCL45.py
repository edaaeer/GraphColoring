...
path4 = 'datasets/myciel4.txt'
path5 = 'datasets/myciel5.txt'
...
def new_generation(edges, population,color):
    new_pop = []
...
def dehb_survivor(edges, population, new_generation):
   fitList = np.empty((0, 3))
   zero_penalty = np.empty((0, 3))
   zero_penalty_ = np.empty((0, 3))
   sorted_values = np.empty((0, 3))
   sorted_values2 = np.empty((0, 3))
   dehb_gen = []
   new_pop_dehb = []
   ...
   for i in population:
       fitness = nfitness(i)
       penaltyvalue = penalty(edges,i)
       if penaltyvalue == 0:
           inner_zero = np.array([sayac, fitness, penaltyvalue])
           zero_penalty = np.vstack([zero_penalty, inner_zero])
           #print("zero penalty",zero_penalty)
       else:
           inner_fit = np.array([sayac, fitness, penaltyvalue])
           fitList = np.vstack([fitList, inner_fit])
           #print("fitList", fitList)
       sayac = sayac + 1

   sayac2 = population_size
   for j in new_generation:
       fitnesschild = nfitness(j)
       penaltychild = penalty(edges, j)
       if penaltychild == 0:
           inner_zero_ = np.array([sayac2, fitnesschild, penaltychild])
           zero_penalty_ = np.vstack([zero_penalty_, inner_zero_])
           #print("zero penalty",zero_penalty_)
       else:
           inner_fit_ = np.array([sayac2, fitnesschild, penaltychild])
           fitList = np.vstack([fitList, inner_fit_])
       sayac2 = sayac2 + 1

    # eğer penaltısı sıfır olan bireyler mevcutsa:
   if len(zero_penalty) > 0:
       # penaltısı 0 olanlar sıralandı.
       size = population_size - len(zero_penalty)

       sorted_indices_zero = np.argsort(zero_penalty[:, 1])
       sorted_pop_zero = zero_penalty[sorted_indices_zero]
       ...
       #print(sorted_values) sorted zero list

   # penaltı değeri 0 olmayanlar, penaltı değerine göre küçükten büyüğe sıralanır
   elif len(fitList) > 0:
       sortedFitList = np.argsort(fitList[:, 2])
       sortedFit = fitList[sortedFitList]
       #print("sortedFit", sortedFit)

       for item2 in sortedFit:
           index,x,y = item2
           inner_sort_ = np.array([index, x, y])
           sorted_values2 = np.vstack([sorted_values2, inner_sort_])

       #print(sorted_values2)
       size = population_size - len(zero_penalty)
       sorted_size = np.array(sorted_values2[:size])
       #print("sortedsize:\n", sorted_size)
       #print(len(sorted_size))

       dehb_gen = np.concatenate((sorted_values, sorted_size))
       #print(dehb_gen)
       #print(len(dehb_gen))
       ...
       for k in dehb_gen[:,0]:
           k = int(k)
           if k < population_size:
               new_pop_dehb.append(population[k])
           else:
               index = k - population_size
               new_pop_dehb.append(new_generation[index])

       new_pop_dehb = np.array(new_pop_dehb)
       #print(new_pop_dehb)
       #print(len(new_pop_dehb))

   return new_pop_dehb

def best_individual(new_gen):
    best_ind = []

    best_ind.append(new_gen[0])
...
if __name__ == '__main__':
    # kullanıcı girişi
    dataset = int(input("Enter dataset(4,5): "))
    if dataset == 4:
        path = path4
    elif dataset == 5:
        path = path5
    ...
    for iter in range(max_iter):
        child = new_generation(edges, population, color)
        ...
        if dataset == 4:
            if best_fit == 5 and best_penalty == 0:
                break
        elif dataset == 5:
            if best_fit == 6 and best_penalty == 0:
                break
